...
import { TrendingUp, TrendingDown, Wind, AlertTriangle, CheckCircle, XCircle, RefreshCw } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const bestCities = [...frenchCities].sort((a, b) => a.airQuality.aqi - b.airQuality.aqi).slice(0, 5);
  const worstCities = [...frenchCities].sort((a, b) => b.airQuality.aqi - a.airQuality.aqi).slice(0, 5);
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-600">Récupération des données en temps réel depuis les APIs</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="p-6">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Qualité de l'Air en France</h2>
            <p className="text-gray-600">Surveillance en temps réel de l'Indice de Qualité de l'Air (IQA) dans les métropoles françaises</p>
          </div>
          ...
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-yellow-800">
                <strong>Attention :</strong> {error}. Les données affichées utilisent des valeurs par défaut.
              </p>
            </div>
          </div>
        )}
          ...
          return (
            <div key={index} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-sm font-medium ${stat.color === 'green' ? 'text-green-600' : stat.color === 'red' ? 'text-red-600' : 'text-gray-600'}`}>
                  {stat.change}
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Meilleure Qualité d'Air</h3>
          </div>
            ...
            {bestCities.map((city, index) => (
              <div key={city.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="font-bold text-green-600">IQA {city.airQuality.aqi}</p>
                  <p className="text-xs text-green-600">{city.airQuality.category}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Qualité d'Air Préoccupante</h3>
          </div>
          <div className="space-y-3">
            {worstCities.map((city, index) => (
              <div key={city.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  <p className="font-bold text-red-600">IQA {city.airQuality.aqi}</p>
                  <p className="text-xs text-red-600">{city.airQuality.category}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-200">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Recommandations du Jour</h3>
            <div className="space-y-2 text-sm text-gray-700">
              <p>• <strong>Activités extérieures :</strong> Limitées pour les personnes sensibles dans {badAirCities} villes</p>
              <p>• <strong>Transport :</strong> Privilégiez les transports en commun et le covoiturage</p>
              <p>• <strong>Ventilation :</strong> Aérez tôt le matin ou tard le soir quand la pollution est moindre</p>
            </div>
          </div>
        </div>
      </div>
